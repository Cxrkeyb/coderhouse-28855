...
router.post('/', async (_req, res) => {
    ...
    newCart
        ? res.status(200).json({"success": "Carrito añadido con ID " + newCart._id})
        : res.status(500).json({"error": "Hubo un error al añadir el carrito"}) 
})
...
router.delete('/:id/productos/:id_prod', async(req, res) => {
    ...
    wasDeleted 
        ...
        : res.status(400).json({"error": "Hubo un problema al eliminar el producto del carrito"})
})
